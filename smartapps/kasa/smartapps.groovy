...
def addDevices() {
	...
	selectedDevices?.each {
    	...
    	if(device == null) {
        	...
        	device = addChildDevice("rainlake", getDeviceType(atomicState.devices[it]), it, null, atomicState.devices[it])
...
def turnDevice(deviceid, state) {
    ...
    if(device != null) {
    	device = device.value
    	log.debug "device.appServerUrl=${device.appServerUrl}"
    	def params = [
        ...
        try {
            httpPostJson(params) { resp ->
                ...
                if(data.error_code == 0) {
                    ...
                    log.debug "relay_state=${data.system.get_sysinfo.relay_state}"
...
def updateDeviceState() {
	...
	getChildDevices().each {
        ...
        if(device != null) {
        	log.debug JsonOutput.toJson(device)
            def newSwitchState = getDeviceState(deviceNetworkId)
            getChildDevice(deviceNetworkId).sendEvent(name: "switch", value: newSwitchState)
        }    	   	
    }
}